...
### 新增
- **财富榜差距惩罚机制**: 防止财富过于集中
  - 新增 `WealthGapPenaltyService` 后台服务，每分钟检测财富榜差距
  - 第一名与第二名差距超过阈值时，第一名获得【厄运】debuff
  - 每小时按动态比例扣除现金（1%-10%，根据差距大小计算）
  - 差距缩小到阈值以下时自动解除 debuff
  - 赋予和去除 debuff 时发送 AT 消息通知用户
  - 新增 `wealth_gap_penalty` 配置组，支持自定义阈值、扣除比例等参数

## v2.19.0 (2026-04-06)

### 新增
- **群主货币交易手续费**: 购买和出售群主货币时扣除手续费
  - 新增 `transaction_fee_rate` 配置项，默认 15%
  - 购买时：实际成本 = 数量 × 汇率 ÷ (1 - 手续费率)
  - 出售时：实际收益 = 数量 × 汇率 × (1 - 手续费率)
  - 交易成功提示显示手续费信息

## v2.18.0 (2026-04-06)

### 修改
- **自动签到优化**: 已签到用户不再收到提示消息
  - 如果用户不在缓存中但已经签到过，静默处理不发送任何消息
  - 减少已签到用户的打扰

### 新增
- **汇率查询持有量限制**: 交易时段内查询汇率需要持有群主货币
  - 新增 `min_currency_for_query` 配置项，默认 1.0
  - 交易时段内查询汇率时检查用户持有的群主货币数量
  - 持有量不足时提示用户需要先购买群主货币
  - 非交易时段不受此限制，可免费查询
...
### 新增
- **自动签到功能**: 支持每天0点后首次发言自动触发签到
  - 新增 `auto_checkin` 配置组，包含 `enabled` 开关
  - 新增 `AutoCheckinService` 类管理自动签到状态
  - 自动签到与手动签到互斥，启用后手动签到指令将提示用户
  - 使用上海时区判断日期变更
  - 自动签到时引用用户原消息并显示完整签到信息
  - 已签到用户不会重复触发自动签到
